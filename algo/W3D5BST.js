...
class BinarySearchTree {
 ...
 insert(newVal){
  ...
  size(count=0, curr = this.root){
    if(!curr){
      return
    }
    count++;
    this.size(count, curr.left);
    this.size(count, curr.left);
  }

  arrInOrder(arr=[], curr=this.root){
    if(!curr){
      return
    }
    this.arrInOrder(arr, curr.left);
    this.arrInOrder(arr, curr.right);
    arr.push(curr.data)
    return arr
  }
...
console.log(insertTree.arrInOrder());